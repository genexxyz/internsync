<x-app-layout>
    <x-breadcrumbs :breadcrumbs="[]" />
    
    <!-- Statistics Cards -->
    <div class="grid grid-cols-1 xl:grid-cols-3 gap-6 p-6">
        <!-- Total Hours Card -->
        <div class="bg-white rounded-xl shadow-sm hover:shadow-lg transition-all duration-300 overflow-hidden">
            <div class="bg-gradient-to-r from-blue-600 to-blue-700 text-white p-6">
                <div class="flex justify-between items-start">
                    <div>
                        <p class="text-4xl font-bold mb-1">
                            {{ $totalHours ?? '00:00' }}/{{ $deployment->custom_hours ?? '00:00' }}
                        </p>
                        <div class="flex items-center gap-2 text-white/90">
                            <i class="fa fa-clock-rotate-left text-sm"></i>
                            <p class="font-medium">Total Hours</p>
                        </div>
                    </div>
                    <div class="bg-white/20 rounded-lg p-3 backdrop-blur-sm">
                        <i class="fa fa-clock text-2xl"></i>
                    </div>
                </div>
            </div>
            <div class="px-6 py-4">
                <a href="{{ route('student.journey') }}" 
                   class="inline-flex items-center px-4 py-2 rounded-lg bg-blue-50 text-blue-700 hover:bg-blue-100 transition-colors gap-2 w-full justify-center">
                    View Details <i class="fa fa-arrow-right"></i>
                </a>
            </div>
        </div>

        <!-- Weekly Reports Card -->
        <div class="bg-white rounded-xl shadow-sm hover:shadow-lg transition-all duration-300 overflow-hidden">
            <div class="bg-gradient-to-r from-violet-600 to-purple-700 text-white p-6">
                <div class="flex justify-between items-start">
                    <div>
                        <p class="text-4xl font-bold mb-1">{{ $weeklyReports->count() }}</p>
                        <div class="flex items-center gap-2 text-white/90">
                            <i class="fa fa-file-pen text-sm"></i>
                            <p class="font-medium">Weekly Reports</p>
                        </div>
                    </div>
                    <div class="bg-white/20 rounded-lg p-3 backdrop-blur-sm">
                        <i class="fa fa-file-lines text-2xl"></i>
                    </div>
                </div>
            </div>
            <div class="px-6 py-4">
                <a href="{{ route('student.taskAttendance') }}" 
                   class="inline-flex items-center px-4 py-2 rounded-lg bg-purple-50 text-purple-700 hover:bg-purple-100 transition-colors gap-2 w-full justify-center">
                    View Reports <i class="fa fa-arrow-right"></i>
                </a>
            </div>
        </div>

        <!-- Attendance Card -->
        <div class="bg-white rounded-xl shadow-sm hover:shadow-lg transition-all duration-300 overflow-hidden">
            <div class="bg-gradient-to-r from-emerald-600 to-green-700 text-white p-6">
                <div class="flex justify-between items-start">
                    <div>
                        <p class="text-4xl font-bold mb-1">{{ $attendanceCount }}</p>
                        <div class="flex items-center gap-2 text-white/90">
                            <i class="fa fa-calendar-check text-sm"></i>
                            <p class="font-medium">Days Present</p>
                        </div>
                    </div>
                    <div class="bg-white/20 rounded-lg p-3 backdrop-blur-sm">
                        <i class="fa fa-calendar-check text-2xl"></i>
                    </div>
                </div>
            </div>
            <div class="px-6 py-4">
                <a href="{{ route('student.taskAttendance') }}" 
                   class="inline-flex items-center px-4 py-2 rounded-lg bg-green-50 text-green-700 hover:bg-green-100 transition-colors gap-2 w-full justify-center">
                    View Attendance <i class="fa fa-arrow-right"></i>
                </a>
            </div>
        </div>
    </div>

    <!-- Notifications Section -->
    <div class="grid grid-cols-1 lg:grid-cols-2 gap-6 px-6 pb-6">
        <!-- Weekly Reports -->
        <div class="bg-white rounded-xl shadow-sm overflow-hidden">
            <div class="bg-gradient-to-r from-gray-800 to-gray-900 text-white p-4">
                <h3 class="font-bold text-lg">Recent Weekly Reports</h3>
            </div>
            <div class="p-4 space-y-3">
                @forelse($weeklyReports as $report)
                    <div class="bg-gray-50 rounded-lg p-4 flex items-center justify-between hover:bg-gray-100 transition-colors border border-gray-100">
                        <div>
                            <p class="text-gray-900 font-semibold">Week {{ $report->week_number }}</p>
                            <p class="text-gray-500 text-sm">
                                {{ Carbon\Carbon::parse($report->start_date)->format('M d') }} - 
                                {{ Carbon\Carbon::parse($report->end_date)->format('M d, Y') }}
                            </p>
                            <span @class([
                                'inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium mt-2',
                                'bg-green-100 text-green-800' => $report->status === 'approved',
                                'bg-yellow-100 text-yellow-800' => $report->status === 'pending',
                                'bg-red-100 text-red-800' => $report->status === 'rejected'
                            ])>
                                {{ ucfirst($report->status) }}
                            </span>
                        </div>
                        <a href="{{ route('student.taskAttendance', $report->id) }}" 
                           class="inline-flex items-center px-4 py-2 rounded-lg bg-blue-50 text-blue-700 hover:bg-blue-100 transition-colors gap-2">
                            View Report <i class="fa fa-arrow-right text-sm"></i>
                        </a>
                    </div>
                @empty
                    <div class="text-center py-8">
                        <div class="bg-gray-50 rounded-lg p-6 border-2 border-dashed border-gray-200">
                            <i class="fa fa-file-circle-question text-gray-400 text-3xl mb-2"></i>
                            <p class="text-gray-500">No weekly reports submitted yet</p>
                        </div>
                    </div>
                @endforelse
            </div>
        </div>

        <!-- Today's Attendance -->
        <div class="bg-white rounded-xl shadow-sm overflow-hidden">
            <div class="bg-gradient-to-r from-gray-800 to-gray-900 text-white p-4">
                <h3 class="font-bold text-lg">Today's Attendance</h3>
            </div>
            <div class="p-4">
                @if($deployment && $deployment->starting_date && $deployment->starting_date->lte(now()))
                    <div class="bg-gray-50 rounded-lg p-4 border border-gray-100">
                        <div class="flex justify-between items-start">
                            <div>
                                <p class="text-gray-900 font-semibold">{{ now()->format('l, F d, Y') }}</p>
                                @if($todayAttendance)
                                    <div class="space-y-1 mt-2">
                                        <p class="text-gray-600 flex items-center gap-2">
                                            <i class="fa fa-clock text-sm w-4"></i>
                                            Time In: {{ $todayAttendance->time_in ? Carbon\Carbon::parse($todayAttendance->time_in)->format('h:i A') : 'Not yet' }}
                                        </p>
                                        @if($todayAttendance->time_out)
                                            <p class="text-gray-600 flex items-center gap-2">
                                                <i class="fa fa-clock text-sm w-4"></i>
                                                Time Out: {{ Carbon\Carbon::parse($todayAttendance->time_out)->format('h:i A') }}
                                            </p>
                                            <p class="text-gray-600 flex items-center gap-2">
                                                <i class="fa fa-hourglass text-sm w-4"></i>
                                                Total Hours: {{ $todayAttendance->total_hours }}
                                            </p>
                                        @endif
                                    </div>
                                @else
                                    <p class="text-gray-500 text-sm mt-1">No attendance record yet</p>
                                @endif
                            </div>
                            <div>
                                @if(!$todayAttendance)
                                    <a href="{{ route('student.taskAttendance') }}" 
                                       class="inline-flex items-center px-4 py-2 rounded-lg bg-blue-600 hover:bg-blue-700 text-white transition-colors gap-2">
                                        <i class="fa fa-clock text-sm"></i> Time In
                                    </a>
                                @elseif(!$todayAttendance->time_out)
                                    <a href="{{ route('student.taskAttendance') }}" 
                                       class="inline-flex items-center px-4 py-2 rounded-lg bg-red-600 hover:bg-red-700 text-white transition-colors gap-2">
                                        <i class="fa fa-clock text-sm"></i> Time Out
                                    </a>
                                @else
                                    <span class="inline-flex items-center px-3 py-1 rounded-full text-sm font-medium bg-green-100 text-green-800">
                                        <i class="fa fa-check-circle mr-1.5"></i> Completed
                                    </span>
                                @endif
                            </div>
                        </div>
                    </div>
                @else
                    <div class="bg-blue-50 rounded-lg p-6 border border-blue-100">
                        <div class="flex items-center gap-3">
                            <div class="flex-shrink-0">
                                <i class="fas fa-circle-info text-blue-500 text-xl"></i>
                            </div>
                            <p class="text-blue-700">
                                Attendance tracking will be available once your internship starts.
                            </p>
                        </div>
                    </div>
                @endif
            </div>
        </div>
    </div>
</x-app-layout>